...
public class LiumDiarizationService extends Thread {
	...
	static ArrayList<LiumDiarizationService> diarizationList;
	 ...
	 */
	public static AudioFeatureSet loadFeature(Parameter parameter, ClusterSet clusterSet, String descriptor)
	 ...
	 *             the diarization exception
	 */
	private AudioFeatureSet loadFeature(AudioFeatureSet featureSet, Parameter parameter, ClusterSet clusterSet,
			String descriptor) throws IOException, DiarizationException {
		String oldDescriptor = parameter.getParameterInputFeature().getFeaturesDescriptorAsString();
		parameter.getParameterInputFeature().setFeaturesDescription(descriptor);
	 ...
	 */
	public ClusterSet sanityCheck(ClusterSet clusterSet, AudioFeatureSet featureSet, Parameter parameter)
			throws DiarizationException, IOException, ParserConfigurationException, SAXException, TransformerException {
	 ...
	 *             the exception
	 */
	public ClusterSet segmentation(String method, String kind, ClusterSet clusterSet, AudioFeatureSet featureSet,
	 ...
	 *             the exception
	 */
	public ClusterSet clusteringLinear(double threshold, ClusterSet clusterSet, AudioFeatureSet featureSet,
			Parameter parameter) throws Exception {
		String mask = parameter.getParameterSegmentationOutputFile().getMask();
	 ...
	 *             the exception
	 */
	public ClusterSet clustering(double threshold, ClusterSet clusterSet, AudioFeatureSet featureSet,
			Parameter parameter) throws Exception {
		String mask = parameter.getParameterSegmentationOutputFile().getMask();
		String oldMethod = parameter.getParameterClustering().getMethodAsString();
		double oldThreshold = parameter.getParameterClustering().getThreshold();
	 ...
	 *             the exception
	 */
	public ClusterSet decode(int nbComp, double threshold, ClusterSet clusterSet, AudioFeatureSet featureSet,
			Parameter parameter) throws Exception {
		String mask = parameter.getParameterSegmentationOutputFile().getMask();
		String oldModelKind = parameter.getParameterModel().getModelKindAsString();
		int oldNumberOfComponent = parameter.getParameterModel().getNumberOfComponents();
		...
		// ** set the penalty to move from the state i to the state j, penalty to move
		// from i to i is equal to 0
	 ...
	 * @param threshold
	 *            the threshold
	 * @param clustersSetBase
	 *            the clusters set base
	 * @param clustersSegInit
	 *            the clusters seg init
	 * @param clustersDClust
	 *            the clusters d clust
	 * @param featureSet
	 *            the feature set
	 * @param parameter
	 *            the parameter
	 ...
	 *             the exception
	 */
	public ClusterSet speech(String threshold, ClusterSet clustersSetBase, ClusterSet clustersSegInit,
			ClusterSet clustersDClust, AudioFeatureSet featureSet, Parameter parameter) throws Exception {
		String mask = parameter.getParameterSegmentationOutputFile().getMask();
		...
		AudioFeatureSet featureSet2 = loadFeature(featureSet, parameter, clustersSetBase,
				FeatureFormat + ",1:3:2:0:0:0,13,0:0:0:0");
		String dir = "ester2";
		// ** load the model : 8 GMMs with 64 diagonal components
		InputStream pmsInputStream = Diarization.class.getResourceAsStream(dir + "/sms.gmms");
		...
		// ** if a speech segment is less than 150 frames, it will be merge to the left
		// or right closest segments
		...
		// ** segments of more than 20s are split according of silence present in the
		// pms or using a gmm silence detector
		InputStream silenceInputStream = Diarization.class.getResourceAsStream(dir + "/s.gmms");
		...
		ClusterSet clustersSplitClust = SSplitSeg.make(featureSet2, clustersFltClust, sVect, clustersPMSClust,
				parameter);
		if (parameter.getParameterDiarization().isSaveAllStep()) {
	 ...
	 * Gender.
	 * 
	 * @param clusterSetBase
	 *            the cluster set base
	 * @param clusterSet
	 *            the cluster set
	 * @param featureSet
	 *            the feature set
	 ...
	 *             the exception
	 */
	public ClusterSet gender(ClusterSet clusterSetBase, ClusterSet clusterSet, AudioFeatureSet featureSet,
			Parameter parameter) throws Exception {
		String mask = parameter.getParameterSegmentationOutputFile().getMask();
		...
		String FeatureFormat = "featureSetTransformation";
		AudioFeatureSet featureSet2 = loadFeature(featureSet, parameter, clusterSet,
				FeatureFormat + ",1:3:2:0:0:0,13,1:1:0:0");
		String dir = "ester2";
		InputStream genderInputStream = Diarization.class.getResourceAsStream(dir + "/gender.gmms");
	 ...
	 *             the exception
	 */
	public ClusterSet speakerClustering(ClusterSet referenceClusterSet, ClusterSet uemClusterSet, String partialKey,
			String method, ClusterSet clusterSet, AudioFeatureSet featureSet, Parameter parameter,
			ClusterSetResultList showResult) throws Exception {
		...
		String FeatureFormat = "featureSetTransformation";
		String dir = "ester2";
		InputStream ubmInputStream = Diarization.class.getResourceAsStream(dir + "/ubm.gmm");
		GMMArrayList ubmVect = MainTools.readGMMContainer(ubmInputStream, parameter.getParameterModel());
		GMM ubm = ubmVect.get(0);

		AudioFeatureSet featureSet2 = loadFeature(featureSet, parameter, clusterSet,
				FeatureFormat + ",1:3:2:0:0:0,13,1:1:300:4");
		parameter.getParameterModel().setModelKind("DIAG");
		...
		while ((score < cMax) && (clustering.getSize() > 1)) {
			...
			/*
			 * if (computeError.isUsed()) { error =
			 * computeError.scoreOfMatchedSpeakers(clustering.getClusterSet()); errorRate =
			 * error.getErrorRate(); logger.fine(parameter.show + " key=" + partialKey +
			 * " clrScore=" + score + " clrErrorRate=" + errorRate + " clrSize=" +
			 * clustering.getSize()+"/"+referenceClusterSet.clusterGetSize()); }
			 */
			if (saveAll) {
				writeCLRClusterSet(clustering.getClusterSet(), nbMerge, parameter);
			}
	 ...
	 *             the transformer exception
	 */
	public static void writeCLRClusterSet(ClusterSet clusterSet, int indexMerge, Parameter parameter)
			throws IOException, ParserConfigurationException, SAXException, DiarizationException, TransformerException {

		String mask = parameter.getParameterSegmentationOutputFile().getMask();
		String mask2 = mask.replace(".seg", "");

		parameter.getParameterSegmentationOutputFile()
				.setMask(mask2 + "." + String.format("%3d", indexMerge).replace(" ", "_") + ".c.seg");
	...
	/*
	 * public synchronized void sumResult(TreeMap<String, DiarizationResultList>
	 * showResult) throws DiarizationException { for (String key :
	 * showResult.keySet()) { DiarizationResultList values = showResult.get(key); if
	 * (corpusResult.containsKey(key)) {
	 * corpusResult.get(key).addResultArray(values); } else { corpusResult.put(key,
	 * values); } } }
	 */
	...
	public boolean isThreadAlive() {
		for (LiumDiarizationService diarization : diarizationList) {
	...
	public void ester2DiarizationCorpus(Parameter parameter) throws DiarizationException, Exception {
		...
		diarizationList = new ArrayList<LiumDiarizationService>(nbThread);

		for (int i = 0; i < nbThread; i++) {
			diarizationList.add(new LiumDiarizationService());
	...
	public void ester2Diarization(Parameter parameter, ClusterSet clusterSet) throws DiarizationException, Exception {
		...
		if (parameter.getParameterDiarization().isLastStepOnly()) {
			...
			if (parameter.getParameterDiarization().isCEClustering() == false) {
				...
				clusterSetResult = speakerClustering(referenceClusterSet, uemClusterSet, key, "ce", clustersSegInit,
						featureSet, parameter, showResult);
			}
		...
		ClusterSet clustersSegSave = segmentation(segmentationMethodString, "FULL", clustersSegInit, featureSet,
				parameter);
		for (double l = lMin; l <= lMax; l += 0.5) {
			...
			for (double h = hMin; h <= hMax; h += 0.5) {
				...
				for (double d = dMin; d <= dMax; d += 50) {
					...
					// clustersDClust);
					ClusterSet clustersSplitClust = speech("10,10,50", clusterSet, clustersSegInit, clustersDClust,
					...
					if (parameter.getParameterDiarization().isCEClustering() == true) {
						clusterSetResult = speakerClustering(referenceClusterSet, uemClusterSet, key, "ce",
								clustersGender, featureSet, parameter, showResult);
					} else {
		...
		// logger.info("value thr:"+dMin+" == "+dMax+") && ("+hMin+" == "+hMax+") &&
		// ("+lMin+" == "+lMax+") ");
		if ((dMin == dMax) && (hMin == hMax) && (lMin == lMax)) {
			MainTools.writeClusterSet(parameter, clusterSetResult, false);
		}
	 ...
	 *            the arguments
	 */
	public static void startDiarization(String[] args) {
		try {
			// Reset All
			diarizationList = new ArrayList<LiumDiarizationService>();

			/** The nb treated job. */
			nbTreatedJob = 0;

			/** The list of cluster set. */
			listOfClusterSet = new ArrayList<ClusterSet>();
			...
			if (parameter.show.isEmpty() == false) {
				LiumDiarizationService diarization = new LiumDiarizationService();
	...
	public static void stopDiarization() {
		// Reset All
		diarizationList = new ArrayList<LiumDiarizationService>();
